...
RUN cd /tmp/vim-esearch && sh spec/support/bin/install_vim_dependencies.sh /tmp/vim_plugins
...
# CMD /bin/bash -l -c "/etc/init.d/xvfb start && . /etc/profile.d/rvm.sh &&  cd /tmp/vim-esearch && rspec --tag nvim"
CMD /bin/bash -l -c "/etc/init.d/xvfb start &&  cd /tmp/vim-esearch && .  /etc/profile.d/rvm.sh && rvm use 2.3.8 && PLUGINS_DIR=/tmp/vim_plugins BIN_DIR=/tmp/bin GUI=0 bundle exec rspec --tag nvim"
