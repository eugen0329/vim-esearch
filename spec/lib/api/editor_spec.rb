...
describe API::Editor, :editor do
  ...
  describe API::Editor::Read::Batch do
    subject(:reader) { described_class.new(editor, method(:vim), true) }
    let(:filename) { 'file.txt' }
    let!(:test_directory) { directory([file("a\nb", filename)]).persist! }
    let(:e) { editor }
    ...
    context 'accept' do
      subject { API::Editor::Read::FlatBatchVisitor.new(reader) }
      let(:ids) { [id1,id2] }
      ...
      context '1d array' do
        ...
        it { expect(result).to eq([[ph0, ph1, 3, '4'], ids, placeholders]) }
      end
      ...
      context '1d hash' do
        ...
        it { expect(result).to eq([{ a: ph0, b: ph1, c: 3, d: 4 }, ids, placeholders]) }
      end

      context 'nested hash' do
        let(:result) { subject.accept(a: e.var(1), b: { c: e.var(2), d: 3, e: 4 }) }

        it { expect(result).to eq([{ a: ph0, b: { c: ph1, d: 3, e: 4 } }, ids, placeholders]) }
      end
    end

    context 'reconstruct' do
      subject { API::Editor::Read::ReconstructVisitor.new(reader) }
