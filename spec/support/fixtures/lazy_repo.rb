...
class Fixtures::LazyRepo
  ...
  def persist!
    ...
    system("git init #{path} ") unless File.directory?(dot_git)
    files.each do |f|
      f.working_directory = directory_path
      f.persist!
      system("git -C #{Shellwords.escape(path)} add #{Shellwords.escape(f.path)}")
      system("git -C #{Shellwords.escape(path)} commit -m '#{Shellwords.escape(f.path)}'")
    end
