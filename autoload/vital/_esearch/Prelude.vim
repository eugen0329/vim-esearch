...
execute join(['function! vital#_esearch#Prelude#import() abort', printf("return map({'escape_pattern': '', 'is_funcref': '', 'path2directory': '', 'wcswidth': '', 'is_string': '', 'input_helper': '', 'is_number': '', 'is_cygwin': '', 'path2project_directory': '', 'strwidthpart_reverse': '', 'input_safe': '', 'is_list': '', 'truncate_skipping': '', 'glob': '', 'truncate': '', 'is_dict': '', 'set_default': '', 'is_numeric': '', 'getchar_safe': '', 'substitute_path_separator': '', 'is_infinity': '', 'is_mac': '', 'strwidthpart': '', 'getchar': '', 'is_unix': '', 'is_windows': '', 'globpath': '', 'is_float': '', 'smart_execute_command': ''}, \"vital#_esearch#function('<SNR>%s_' . v:key)\")", s:_SID()), 'endfunction'], "\n")
delfunction s:_SID
" ___vital___
let s:save_cpo = &cpo
set cpo&vim
...
function! s:_path2project_directory_others(marker, search_directory) abort
  let marker = a:marker
  let search_directory = a:search_directory

  let find_directory = s:_escape_file_searching(search_directory)
  let d = finddir(marker, find_directory . ';')
  if d ==# ''
    return ''
  endif
...
function! s:path2project_directory(path, markers) abort
  ...
  for marker in a:markers
    ...
    elseif marker ==# '.svn'
      ...
      let directory = s:_path2project_directory_others(marker, search_directory)
    endif

    if directory ==# ''
      let file = findfile(marker, s:_escape_file_searching(search_directory) . ';')
      if file !=# ''
